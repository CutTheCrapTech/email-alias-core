...
- Tests the main `generateEmailAlias` and `validateEmailAlias` functions
...
The library uses a sophisticated crypto detection system:

1. **Modern Browsers/Workers**: Uses `globalThis.crypto.subtle`
2. **Older Browsers**: Falls back to `window.crypto.subtle`
3. **Web Workers**: Uses `self.crypto.subtle`
4. **Node.js 16+**: Uses `require("node:crypto").webcrypto`
